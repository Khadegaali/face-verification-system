...
UPLOAD_FOLDER = os.path.join(BASE_DIR, "uploads")  # folder to save user photos
DB_PATH = os.path.join(BASE_DIR, "face_verification.db") # path to the SQLite database
...
def ensure_rgb_and_enhance(path):
    try:
        img = Image.open(path)
        if img.mode != "RGB":
            img = img.convert("RGB")   # convert RGBA/grayscale toRGB
...
def register():
    ...
    try:
        user_id = save_user(name, email, filename)  
    except Exception as e:
